...
public
class AntiCrystal extends ListenerAntiCrystal {
    private final List < BlockPos > targets = new ArrayList <> ( );
    private final Timer timer = new Timer ( );
    private final Timer breakTimer = new Timer ( );
    private final Timer checkTimer = new Timer ( );
    protected final Setting<Float> range = register(new NumberSetting<>("Range", 6.0f , 0.0f , 10.0f));
    protected final Setting < Float > wallsRange = register ( new NumberSetting <> ( "WallsRange" , 3.5f , 0.0f , 10.0f ) );
    protected final Setting < Float > minDmg = register ( new NumberSetting <> ( "MinDmg" , 6.0f , 0.0f , 40.0f ) );
    protected final Setting < Float > selfDmg = register ( new NumberSetting <> ( "SelfDmg" , 2.0f , 0.0f , 10.0f ) );
    protected final Setting < Integer > placeDelay = register ( new NumberSetting <> ( "PlaceDelay" , 0 , 0 , 500 ) );
    protected final Setting < Integer > breakDelay = register ( new NumberSetting <> ( "BreakDelay" , 0 , 0 , 500 ) );
    protected final Setting < Integer > checkDelay = register ( new NumberSetting <> ( "CheckDelay" , 0 , 0 , 500 ) );
    protected final Setting < Integer > wasteAmount = register ( new NumberSetting <> ( "WasteAmount" , 1 , 1 , 5 ) );
    protected final Setting < Switch > switcher = register ( new EnumSetting<>( "Switch" , Switch.NONE ) );
    protected final Setting < Updates > mode = register ( new EnumSetting<> ( "Updates" , Updates.TICK ) );
    protected final Setting < Boolean > rotate = register ( new BooleanSetting ( "Rotate" , true ) );
    protected final Setting < Boolean > packet = register ( new BooleanSetting ( "Packet" , true ) );
    protected final Setting < Boolean > instant = register ( new BooleanSetting ( "Predict" , false ) );
    protected final Setting < Boolean > resetBreakTimer = register ( new BooleanSetting ( "ResetBreak" , true ) );
    protected final Setting < Integer > rotations = register ( new NumberSetting ( "Spoofs" , 1 , 1 , 20 ) );
    private float yaw;
    private float pitch;
