...
public class ConfigManager extends AbstractRegister<ConfigHelper<?>> {
    ...
    public void saveAll() throws IOException {
        for (ConfigHelper<?> helper : getRegistered()) {
            Chulohack.getLogger().info("Config saving: " + helper.getName());
            helper.save();
        }

        Chulohack.getLogger().info("Config saving: current");
        CurrentConfig.getInstance().save();
    }

    public void refreshAll() throws IOException {
        Chulohack.getLogger().info("Config refreshing: current");
        CurrentConfig.getInstance().refresh();
        for (ConfigHelper<?> helper : getRegistered()) {
            Chulohack.getLogger().info("Config refreshing: " + helper.getName());
            helper.refresh();
    ...
    public void load(ConfigHelper<?> helper, String... args) throws IOException {
        Chulohack.getLogger().info("Config: loading " + helper.getName() + " "
                + Arrays.toString(args));
        execute(helper, h -> {
        }, ConfigHelper::load, args);
    }
