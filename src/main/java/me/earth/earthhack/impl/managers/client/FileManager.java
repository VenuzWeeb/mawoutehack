                ...
                if (file.getName().endsWith("gif")) {
                    if (!gifs.containsKey(file.getName())) {
                        try {
                            ...
                            Chulohack.getLogger().error("Failed to load gif image " + file.getName() + "!");
                            e.printStackTrace();
                        }
                    }
                } else {
                    try {
                        ...
                        Chulohack.getLogger().error("Failed to load image " + file.getName() + "!");
                        e.printStackTrace();
                    }
                }
            }
        ...
        if (dir.isDirectory()) {
            for (File file : Objects.requireNonNull(dir.listFiles())) {
                try {
                    ...
                    Chulohack.getLogger().error("Failed to shader model: " + file.getName() + "!");
                    e.printStackTrace();
                }
            }
        }
